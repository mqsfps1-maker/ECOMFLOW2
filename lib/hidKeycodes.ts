// lib/hidKeycodes.ts

// Maps HID Key Codes (from USB HID Usage Tables) to US keyboard layout characters.
// [normal_char, shift_char]
export const HID_KEY_CODES_US: { [key: number]: [string, string] } = {
    4: ['a', 'A'], 5: ['b', 'B'], 6: ['c', 'C'], 7: ['d', 'D'], 8: ['e', 'E'],
    9: ['f', 'F'], 10: ['g', 'G'], 11: ['h', 'H'], 12: ['i', 'I'], 13: ['j', 'J'],
    14: ['k', 'K'], 15: ['l', 'L'], 16: ['m', 'M'], 17: ['n', 'N'], 18: ['o', 'O'],
    19: ['p', 'P'], 20: ['q', 'Q'], 21: ['r', 'R'], 22: ['s', 'S'], 23: ['t', 'T'],
    24: ['u', 'U'], 25: ['v', 'V'], 26: ['w', 'W'], 27: ['x', 'X'], 28: ['y', 'Y'],
    29: ['z', 'Z'], 30: ['1', '!'], 31: ['2', '@'], 32: ['3', '#'], 33: ['4', '$'],
    34: ['5', '%'], 35: ['6', '^'], 36: ['7', '&'], 37: ['8', '*'], 38: ['9', '('],
    39: ['0', ')'], 44: [' ', ' '], 45: ['-', '_'], 46: ['=', '+'], 47: ['[', '{'],
    48: [']', '}'], 49: ['\\', '|'], 51: [';', ':'], 52: ['\'', '"'], 53: ['`', '~'],
    54: [',', '<'], 55: ['.', '>'], 56: ['/', '?'],
    // Numpad
    89: ['1', '1'], 90: ['2', '2'], 91: ['3', '3'], 92: ['4', '4'], 93: ['5', '5'],
    94: ['6', '6'], 95: ['7', '7'], 96: ['8', '8'], 97: ['9', '9'], 98: ['0', '0'],
    85: ['*', '*'], 86: ['-', '-'], 87: ['+', '+'], 84: ['/', '/'], 99: ['.', '.']
};
