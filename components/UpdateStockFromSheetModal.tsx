// components/UpdateStockFromSheetModal.tsx
import React, { useState, useMemo } from 'react';
// FIX: Import XLSX to resolve 'Cannot find name XLSX' error.
...
interface UpdateStockFromSheetModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const UpdateStockFromSheetModal: React.FC<UpdateStockFromSheetModalProps> = ({ isOpen, onClose, stockItems, onBulkInventoryUpdate }) => {
    ...
    const searchResults = useMemo<StockItem[]>(() => {
        ...
        // FIX: Explicitly type the parameter 'si' to resolve a type inference issue where it was being treated as 'unknown'.
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // FIX: Explicitly type jsonData as any[] to avoid incorrect type inference that leads to misleading errors.
            ...
            const parsedSheetData: SheetRow[] = jsonData
                .filter((r): r is Record<string, any> => typeof r === 'object' && r !== null) // Ensure we only process objects
                .map(row => ({
            ...
            // Auto-linking logic
            // FIX: Replaced .map with a for...of loop to work around type inference issues with array methods.
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-4xl flex flex-col max-h-[90vh]">
                <div className="flex-shrink-0 flex justify-between items-center mb-4 border-b pb-4">
                    <h2 className="text-xl font-bold text-gray-800">Inventário Inicial por Planilha</h2>
...
export default UpdateStockFromSheetModal;